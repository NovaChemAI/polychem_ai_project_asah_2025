    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-border overflow-hidden flex flex-col md:flex-row transition-all">
        ...
        <div className="p-8 md:p-10 md:w-2/3 flex flex-col">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/80 p-4 rounded-xl mb-8 font-mono text-xs text-slate-500 dark:text-slate-400 break-all border border-slate-200 dark:border-slate-700 relative group">
            <span className="absolute -top-2 left-3 bg-white dark:bg-slate-900 px-2 text-[9px] font-bold text-slate-400 border border-slate-200 dark:border-slate-700 rounded uppercase">SMILES Notation</span>
            {compound.smiles}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            ...
            <div className="p-5 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-800/50 col-span-1 sm:col-span-2">
              ...
              <p className="text-4xl font-black text-slate-900 dark:text-white mb-2">{compound.tg} °C</p>
      ...
      <div className="mt-16">
        <div className="flex items-center gap-4 mb-8">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">Similar Compounds Found</h2>
            <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800"></div>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {similar.map((item, idx) => (
            <div key={idx} className="bg-white dark:bg-slate-900 flex flex-col p-6 rounded-2xl border border-slate-200 dark:border-slate-800 hover:shadow-2xl hover:border-blue-300 dark:hover:border-blue-800 transition-all duration-300 group">
                {/* Badge Match */}
                <div className="flex justify-between items-center mb-5">
                    ...
                    <span className="text-slate-300 dark:text-slate-700 font-black text-xs">#{idx + 1}</span>
                </div>
                ...
                <div className="h-44 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 mb-6 p-4 flex items-center justify-center overflow-hidden relative">
                    <img 
                        src={`${KOYEB_BASE_URL}${item.image_url}`} 
                        alt={item.name} 
                ...
                {/* Info Senyawa */}
                <div className="mb-5">
                    <h3 className="font-black text-lg text-slate-900 dark:text-white mb-1 truncate" title={item.name}>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                    <div className="p-3 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800">
                        <p className="text-[9px] uppercase font-black text-slate-400 tracking-widest mb-1">Mol. Wt</p>
                        <p className="font-bold text-slate-800 dark:text-slate-200 text-sm">{item.molecular_weight.toFixed(2)}</p>
                    </div>
                    <div className="p-3 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800">
                        <p className="text-[9px] uppercase font-black text-slate-400 tracking-widest mb-1">Tg Predict</p>
                        <p className="font-bold text-slate-800 dark:text-slate-200 text-sm">{item.tg} °C</p>
                    </div>
                </div>
                ...
                <div className="mt-auto pt-5 border-t border-dashed border-slate-200 dark:border-slate-800">
                    <div className="flex items-start gap-2">
                        <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${item.tg > 50 ? 'bg-orange-400' : 'bg-blue-400'}`}></div>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed italic">
                            Struktur ini teridentifikasi memiliki kemiripan fungsional tinggi dengan target novel compound, menunjukkan karakteristik termal stabil pada {item.tg} °C.
                        </p>
                    </div>
                </div>
            </div>
