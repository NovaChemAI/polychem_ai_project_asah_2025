...
function DashboardLayout({ children }: LayoutProps) {
  ...
  // 3. FUNGSI TRIGGER MODAL (Bukan langsung logout)
  const handleLogoutClick = () => {
    setIsLogoutModalOpen(true);
  };

  // 4. FUNGSI EKSEKUSI LOGOUT (Setelah konfirmasi)
    ...
    <div className="flex h-screen bg-gray-50 dark:bg-slate-900 overflow-hidden transition-colors duration-300">
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 hidden md:flex flex-col justify-between transition-all duration-300 ease-in-out`}>
        ...
        <div className={`h-16 flex items-center border-b border-gray-200 dark:border-slate-700 shrink-0 transition-all duration-300 ${isSidebarOpen ? 'px-6 justify-between' : 'px-0 justify-center'}`}>
            <div className="flex items-center overflow-hidden">
              <div className="h-10 w-10 rounded-xl overflow-hidden flex items-center justify-center bg-transparent flex-shrink-0 transition-colors duration-300">
                  <img src={logoImage} alt="Logo" className="h-full w-full object-contain" />
              </div>
              <span className={`font-bold text-gray-900 dark:text-white text-lg tracking-tight ml-3 transition-opacity duration-300 whitespace-nowrap ${isSidebarOpen ? 'opacity-100' : 'opacity-0 w-0 hidden'}`}>
                PolyChemAI
              </span>
            </div>
             <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={`text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors ${!isSidebarOpen ? 'hidden' : 'block'}`}>
               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 19l-7-7 7-7m8 14l-7-7 7-7" /></svg>
             </button>
        </div>

        {!isSidebarOpen && (
           <button onClick={() => setIsSidebarOpen(true)} className="w-full flex justify-center py-2 text-gray-400 hover:text-blue-600 border-b border-gray-100 dark:border-slate-700">
        ...
        <div className="p-4 border-t border-gray-200 dark:border-slate-700 space-y-1">
           ...
           <MenuButton
             ...
             onClick={user ? handleLogoutClick : () => navigate('/login')} // Ubah onClick
      ...
      <main className="flex-1 overflow-y-auto relative dark:text-gray-100">
        <div className="md:hidden absolute top-4 left-4 z-20">
           <button className="p-2 bg-white dark:bg-slate-800 rounded-md shadow-md border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-200">☰</button>
        </div>
        <div className="p-8 min-h-full">{children}</div>
      </main>

      {/* 5. RENDER MODAL DI SINI (Overlay) */}
      <ConfirmationModal
        isOpen={isLogoutModalOpen}
        title="Konfirmasi Keluar"
        message="Apakah Anda yakin ingin keluar dari akun? Anda harus login kembali untuk mengakses data yang tersimpan."
