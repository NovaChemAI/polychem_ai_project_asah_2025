...
const MenuItem = ({
  ...
  >
    ...
    <span
      className={`ml-4 transition-opacity duration-300 whitespace-nowrap ${isOpen ? "opacity-100" : "opacity-0 w-0 hidden"}`}
    >
      {label}
    </span>
  </Link>
...
// Komponen User Section Reusable (dipakai di sidebar desktop & drawer mobile)
const UserSection = ({
  isOpen,
  user,
  onLogoutClick,
...
function DashboardLayout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-slate-900 overflow-hidden transition-colors duration-300">
      {/* SIDEBAR DESKTOP */}
      <aside
        className={`${isSidebarOpen ? "w-64" : "w-20"} bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 hidden md:flex flex-col justify-between transition-all duration-300 ease-in-out`}
      >
        <div
          className={`h-16 flex items-center border-b border-gray-200 dark:border-slate-700 shrink-0 transition-all duration-300 ${isSidebarOpen ? "px-6 justify-between" : "px-0 justify-center"}`}
        >
          <div
            className={`flex items-center overflow-hidden ${!isSidebarOpen ? "cursor-pointer hover:opacity-70 transition-opacity" : ""}`}
      ...
      <main className="flex-1 overflow-y-auto relative dark:text-gray-100">
        ...
        <div className="p-4 md:p-8 min-h-full">{children ?? <Outlet />}</div>
      </main>
